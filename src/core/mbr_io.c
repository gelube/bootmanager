/**
 * mbr_io.c - Native MBR repair without bootrec.exe
 *
 * Writes standard Windows MBR bootstrap code (first 446 bytes) to PhysicalDriveN,
 * preserving the partition table (bytes 446-511).
 */

#include "../../include/mbr_io.h"
#include <stdio.h>
#include <string.h>

/*
 * Standard Windows 7/8/10/11 MBR bootstrap code (exactly 446 bytes).
 * Extracted from a clean Windows 10 installation.
 * Only the bootstrap code — partition table is preserved from the target disk.
 */
static const BYTE s_winMbr[446] = {
    
};

BOOL RepairMBR_Native(int diskNumber)
{
    WCHAR path[32];
    swprintf(path, 32, L"\\\\.\\PhysicalDrive%d", diskNumber);

    HANDLE hDisk = CreateFileW(path,
        GENERIC_READ | GENERIC_WRITE,
        FILE_SHARE_READ | FILE_SHARE_WRITE,
        NULL, OPEN_EXISTING, 0, NULL);

    if (hDisk == INVALID_HANDLE_VALUE)
        return FALSE;

    /* Read current MBR to preserve partition table (bytes 446-511) */
    BYTE mbr[512];
    DWORD bytes;
    if (!ReadFile(hDisk, mbr, 512, &bytes, NULL) || bytes != 512) {
        CloseHandle(hDisk);
        return FALSE;
    }

    /* Replace bootstrap code only, keep partition table + signature */
    memcpy(mbr, s_winMbr, 446);

    /* Write back */
    SetFilePointer(hDisk, 0, NULL, FILE_BEGIN);
    BOOL ok = WriteFile(hDisk, mbr, 512, &bytes, NULL) && bytes == 512;

    CloseHandle(hDisk);
    return ok;
}
